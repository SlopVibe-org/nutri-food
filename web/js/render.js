...
async function loadSelectionsFromServer() {
 try {
 let res = await fetchWithTimeout(API + '/selections', { headers: { } }, 10000);
...
function renderSimple() {
 ...
 // Weekly nutrition objectives card
 ...
 // Fetch and render weekly nutrition objectives
 _renderSimpleWeeklyNutrition(function() { openResetConfirm(); });
...
// ─── Weekly nutrition objectives for simple view ───
async function _renderSimpleWeeklyNutrition(resetCallback) {
 ...
 if (!res.ok) return;
 let data = await res.json();
 let weekTotals = data.week_totals || {};
 ...
 '<span style="font-size:0.75rem;color:var(--text-dim);">📊 Objectifs nutritionnels (semaine)</span>' +
